...
function dmb_rpts_plan_meta_box_save($post_id)
{
    ...
    for ($i = 0; $i < $count; ++$i) {
        if (
            ...
            if (isset($_POST['plan_custom_buttons'][$i]) && $_POST['plan_custom_buttons'][$i]) {
                // let admins/editors use unfiltered html
                if (current_user_can('unfiltered_html')) {
                    $new_plans[$i]['_rpt_btn_custom_btn'] = balanceTags($_POST['plan_custom_buttons'][$i]);
                } else {
                    $new_plans[$i]['_rpt_btn_custom_btn'] = balanceTags(wp_kses_post($_POST['plan_custom_buttons'][$i]));
                }
            } else {
                $new_plans[$i]['_rpt_btn_custom_btn'] = '';
            }
